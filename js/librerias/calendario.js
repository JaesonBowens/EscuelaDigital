jQuery.fn.calendarioDW = function() {
   this.each(function(){
		...
		var dias = ["m", "t", "w", "t", "f", "s", "s"];
		//nombres de los meses
		var nombresMes = ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"]
		...
		function mostrarCalendario(){
			if(!mostrando){
				...
				var botonCerrar = $('<a href="#" class="calencerrar"><img src="../../imagen/icon/101.png"/></a>');
		...
		function muestraDiasMes(mes, ano){
			...
			capaDiasMes.children().click(function(e){
				...
				if (numDiaPulsado != ""){
					...
					else if(elem.attr("id") == "fecha_nac"){
						var fecha_nac = elem.val().split("/");
						var fecha_hoy = new Date();

						var fecha_nac_validar = new Date(fecha_nac[2], fecha_nac[1] - 1, fecha_nac[0]);

       						if(fecha_nac_validar > fecha_hoy){

            						alert("D.O.B. must be before today´s date");

							elem.val("");

        					}else {

           						var edad = fecha_hoy.getFullYear() - fecha_nac[2];
							...
							if(edad >= 6 && edad < 16){
								...
								alert("Can´t register a student that´s less than 6 years old o more than 16 years old");
								elem.val("");
								$("#edad").val("");
							}
        					} 

					}else if(elem.attr("id") == "fecha_prueba"){
						var fecha_prueba = elem.val().split("/");
						var fecha_hoy = new Date();

						var fecha_prueba_validar = new Date(fecha_prueba[2], fecha_prueba[1] - 1, fecha_prueba[0]);

       						if(fecha_prueba_validar > fecha_hoy){

            						alert("The date of the test must be before today´s date");
