<?php
	...
	if(isset($_POST["consultarActividad"])){
		...
		$nombre_actividad = "SPELL THE NUMBER";

		//Verificamos que no posea iteraciones
		if(!isset($_SESSION["act4_consultar_imagenes"]) || $_SESSION['act4_consultar_imagenes'] == true){

			//Asignamos la primera iteracion
			//$_SESSION["iteracion"] = 1;
			$id_imagen = array();
			...
			if($resulA){
				...
				$_SESSION["act4_id_imagen"] = $id_imagen;
			}
			else{
		...
		if(count($_SESSION["act4_id_imagen"]) != 0){
			//Extraemos el ultimo elemento del arreglo
			$id_imag = array_pop($_SESSION["act4_id_imagen"]);
			...
			if($resulB){
				...
				$actividad["lng_sign"] = $filaB["lenguaje_sena"];
				$actividad["audio"] = $filaB["direccion_audio"];
				$actividad["respuesta"]  = $filaB["palabra"];
		...
		$intento_sin_exito	= $arreglo->intentoP;
		...
		$iteracion = $_SESSION["act4_iteracion_prueba"];
		...
		if($resulC){
			...
			if($resulD){
				...
				if($id_prueba == false){
					...
					$_SESSION["act4_iteracion_prueba"] = $iteracion + 1;
					$_SESSION["act4_consultar_imagenes"] = false;
		...
		/*$contado = $modeloEjercicioRealizado -> contar($id_prueba);
