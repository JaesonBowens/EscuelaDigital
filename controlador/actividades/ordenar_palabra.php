<?php
	...
	if(isset($_POST["consultarActividad"])){
		...
		$nombre_actividad = "ARRANGE THE WORD";

		//Verificamos que no posea iteraciones
		if(!isset($_SESSION["act6_consultar_imagenes"]) || $_SESSION['act6_consultar_imagenes'] == true){
			$id_imagen = array();
			...
			if($resulA){
				...
				$_SESSION["act6_id_imagen"] = $id_imagen;
			}
			else{
		...
		if(count($_SESSION["act6_id_imagen"]) != 0){
			//Extraemos el ultimo elemento del arreglo
			$id_imag = array_pop($_SESSION["act6_id_imagen"]);
		...
		$iteracion = $_SESSION["act6_iteracion_prueba"];
		...
		if($resulC){
			...
			if($resulD){
				...
				if($id_prueba == false){
					...
					$_SESSION["act6_iteracion_prueba"] = $iteracion + 1;
					$_SESSION["act6_consultar_imagenes"] = false;
