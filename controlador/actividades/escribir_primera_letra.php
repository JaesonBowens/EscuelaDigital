<?php
	...
	if(isset($_POST["consultarActividad"])){
		...
		$nombre_actividad = "WRITE THE FIRST LETTER OF THE OBJECT";

		//Verificamos que no posea iteraciones
		if(!isset($_SESSION["act10_consultar_imagenes"]) || $_SESSION['act10_consultar_imagenes'] == true){
			...
			if($resulA){
				...
				$_SESSION["act10_id_imagen"] = $id_imagen;
			}
			else{
		...
		if(isset($_SESSION["act10_id_imagen"]) && count($_SESSION["act10_id_imagen"]) != 0){
			//Extraemos el ultimo elemento del arreglo
			$id_imag = array_pop($_SESSION["act10_id_imagen"]);
		...
		$iteracion = $_SESSION["act10_iteracion_prueba"];
		...
		if($resulC){
			...
			if($resulD){
				...
				if($id_prueba == false){
					...
					$_SESSION["act10_iteracion_prueba"] = $iteracion + 1;
					$_SESSION["act10_consultar_imagenes"] = false;
