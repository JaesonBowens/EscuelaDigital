<?php
	...
	include("../cabecera/campos_de_seleccion/operacion_sobre_tabla.php");
...
<head>
    ...
    <script language="javascript" type="text/javascript" src="../js/validacion.js"></script>
		...
		<script language="javascript" type="text/javascript" src="../js/bitacora.js"></script>
		...
		<script type="text/javascript">
				$(document).ready(function(){
					...
					$("#limpiar_campo_fecha").click(function(){
							...
							$("#buscar_fecha").val("");
							busquedaAvanzadaBitacora('', 'cuerpo-tabla-maestro', '../controlador/bitacora.php', 'consultarActualizacion')
		...
		<script>
	$(document).ready(function(){
		$('#contenido_principal').css({ 'height':$(window).height() - $(window).height() /  });
		$('#pie').css({ 'margin-top': 180});
	});
    </script>
</head>

<body onLoad="busquedaAvanzadaBitacora('', 'cuerpo-tabla-maestro', '../controlador/bitacora.php', 'consultarActualizacion')" >
...
<div id="contenido_principal" >
	<section class="nombre_maestro">
		<h5>Bit&aacute;cora</h5>
	</section>

	<form action="../controlador/bitacora.php" method="POST">

		<fieldset id="catalogo-maestro" >
							<legend>Realized transaction by the users of the system</legend>
										<section class="buscar-maestro-label">
												<label for="buscar">Date:</label>
												<input type="text" id="buscar_fecha" name="buscar_fecha" class="calendario" value="" size="10px" title="Enter the date to search for when the operation was realized" maxlength="25" onKeyup="busquedaAvanzadaBitacora('', 'cuerpo-tabla-maestro', '../controlador/bitacora.php', 'consultarActualizacion'); validarLongitud(this.value, '11')" /> <a id="limpiar_campo_fecha" class='limpiar_ico' href='#' title="Press to clean date field"><span></span></a>

											<label for="buscar">Table:</label>
												<select id="buscar_tabla" name="buscar_tabla" title="Seleccione la tabla" onChange="busquedaAvanzadaBitacora('', 'cuerpo-tabla-maestro', '../controlador/bitacora.php', 'consultarActualizacion')" >
											...
											<label for="buscar">Operation:</label>
 												<select id="buscar_operacion" name="buscar_operacion" title="Select the operation" onChange="busquedaAvanzadaBitacora('', 'cuerpo-tabla-maestro', '../controlador/bitacora.php', 'consultarActualizacion')" >
												<option value="">SELECT</option>
												<?php	
													cargarOperacionSobreTabla()
												?>
											</select>
