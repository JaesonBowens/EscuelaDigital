<?php
	...
	if(isset($_POST["consultarActividad"])){
		...
		$nombre_actividad = "IDENTIFY THE NUMBER";

		//Verificamos que no posea iteraciones
		if(!isset($_SESSION["act3_consultar_imagenes"]) || $_SESSION['act3_consultar_imagenes'] == true){
			...
			if($resulA){
				...
				$_SESSION["act3_id_imagen"] = $id_imagen;
			}
			else{
		...
		if(count($_SESSION["act3_id_imagen"]) != 0){
			//Extraemos el ultimo elemento del arreglo
			$id_imag = array_pop($_SESSION["act3_id_imagen"]);
			...
			if($resulB){
				...
				$actividad["respuesta"]  = $filaB["numero"];
		...
		$iteracion = $_SESSION["act3_iteracion_prueba"];
		...
		if($resulC){
			...
			if($resulD){
				...
				if($id_prueba == false){
					...
					$_SESSION["act3_iteracion_prueba"] = $iteracion + 1;
					$_SESSION["act3_consultar_imagenes"] = false;
