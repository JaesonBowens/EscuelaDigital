<?php
	...
	if(isset($_POST["consultarActividad"])){
		...
		$nombre_actividad = "SUBTRACT THE OBJECTS";

		//Verificamos que no posea iteraciones
		if(!isset($_SESSION["act12_consultar_imagenes"]) || $_SESSION['act12_consultar_imagenes'] == true){
			...
			if($resulA){
				...
				$_SESSION["act12_id_agrupacion"] = $id_agrupacion;
		...
		if(count($_SESSION["act12_id_agrupacion"]) != 0){
			$id_agrupacion_consultar = array_pop($_SESSION['act12_id_agrupacion']);
		...
		$iteracion = $_SESSION["act12_iteracion_prueba"];
		...
		if($resulC){
			...
			if($resulD){
				...
				if($id_prueba == false){
					...
					$_SESSION["act12_iteracion_prueba"] = $iteracion + 1;
					$_SESSION["act12_consultar_imagenes"] = false;
